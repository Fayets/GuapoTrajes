...
export default function ReportesPage() {
  const [selectedReporte, setSelectedReporte] = useState<string>("alquileres_por_prenda");
  const { token } = useAuth();
  ...
  const [filtroVeces, setFiltroVeces] = useState<"todos" | "cero" | "menos_10" | "mas_10">("todos");
  ...
  const [filtroFechaContratos, setFiltroFechaContratos] = useState<"fecha_creacion" | "fecha_evento">("fecha_creacion");
  const [tipoContrato, setTipoContrato] = useState<"todos" | "presupuestos" | "ordenes_trabajo">("todos");
  ...
  const exportarRecibosCSV = () => {
    ...
    const rows = recibos.map((item) => [
      format(new Date(item.fecha_hora + "T00:00:00"), "dd/MM/yyyy HH:mm", { locale: es }),
  ...
  const exportarIngresosCSV = () => {
    if (!ingresosPorTipo || !ingresosPorTipo.ingresos_por_tipo || ingresosPorTipo.ingresos_por_tipo.length === 0) {
      toast.error("No hay datos para exportar");
      return;
    }
  ...
  const actualizarEstadoProducto = async (productoId: number, nuevoEstado: string) => {
    try {
      const response = await fetch(
  ...
  const obtenerStockPorEstado = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/reportes/stock-por-estado`,
        {
          headers: getAuthHeaders(),
        }
      );
  ...
  const exportarStockCSV = () => {
    if (!stockPorEstado || !stockPorEstado.stock_por_estado || stockPorEstado.stock_por_estado.length === 0) {
      toast.error("No hay datos para exportar");
      return;
    }
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `stock_por_estado_${new Date().toISOString().split("T")[0]}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
  ...
  const obtenerStockPorLinea = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/reportes/stock-por-linea`,
        {
          headers: getAuthHeaders(),
        }
      );
  ...
  const exportarStockLineaCSV = () => {
    if (!stockPorLinea || !stockPorLinea.stock_por_linea || stockPorLinea.stock_por_linea.length === 0) {
      toast.error("No hay datos para exportar");
      return;
    }
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `stock_por_linea_${new Date().toISOString().split("T")[0]}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
...
<body>
    ...
    <div class="recibo-body">
        ...
        <div class="detalles">
            ...
            <p><strong>Cliente:</strong> ${recibo.cliente_nombre} (DNI: ${recibo.cliente_dni})</p>
    ...
    if (ventana) {
      ...
      toast.error("Por favor, permite ventanas emergentes para imprimir el recibo");
    }
  };
        ...
        <div className="card">
          <div className="card-header">
            ...
            <p className="text-muted small mb-0">
              Cantidad de veces que cada prenda fue alquilada en un rango de fechas
            </p>
          </div>
          <div className="card-body">
            ...
            <div className="row mb-4">
              ...
              <div className="col-md-4 d-flex align-items-end gap-2">
                ...
                {alquileres.length > 0 && (
                  <button onClick={exportarCSV} className="btn btn-outline-success">
                      ...
                      <div className="card border-success">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-success fw-bold">
                            {alquileres.reduce(
                              (sum, item) => sum + item.cantidad_total_alquilada,
                              0
                            )}
                          </div>
                        </div>
          ...
          <div className="card-body">
            ...
            {isLoadingRanking ? (
              ...
              const productosFiltrados = ranking.filter((item) => {
                ...
                if (filtroVeces === "menos_10") return veces > 0 && veces < 10;
                ...
                return (
                  <div className="text-center text-muted py-5">
                    ...
                    <p className="text-muted">
                      ...
                      <br />
                      <small>Verifica que haya productos en tu sucursal y que las fechas sean correctas.</small>
                    </p>
                  </div>
                );
              }
              ...
              if (productosFiltrados.length === 0) {
                return (
                  <div className="text-center text-muted py-5">
                    ...
                    <h5 className="text-muted">No hay resultados con el filtro seleccionado</h5>
                    <p className="text-muted">
                      Hay {ranking.length} productos en total, pero ninguno cumple con el filtro "{filtroVeces === "cero" ? "Cero veces" : filtroVeces === "menos_10" ? "Menos de 10 veces" : "Más de 10 veces"}".
                      <br />
                      <small>Intenta cambiar el filtro o verifica las fechas seleccionadas.</small>
                    </p>
                  </div>
                );
              }
                          ...
                          <div className="text-primary small mb-1">
                            ...
                            {filtroVeces !== "todos" && (
                              <span className="text-muted ms-2">
                                (Filtrado: {filtroVeces === "cero" ? "Cero veces" : filtroVeces === "menos_10" ? "Menos de 10 veces" : "Más de 10 veces"})
                              </span>
                            )}
                          </div>
                  ...
                  <table className="table table-hover table-striped">
                    ...
                    <tbody>
                      {productosPaginados.map((item) => {
                        const nuncaAlquilado = item.cantidad_veces_alquilada === 0 && item.cantidad_total_alquilada === 0;
                        return (
                          <tr 
                            key={item.producto_id}
                            className={nuncaAlquilado ? "table-secondary opacity-75" : ""}
                          >
                            <td className="text-center">
                              <span className="badge bg-light text-dark border" style={{ fontSize: "0.75rem", padding: "0.25rem 0.5rem" }}>
                            ...
                            <td className={`text-center fw-semibold ${nuncaAlquilado ? "text-muted" : "text-success"}`}>
                              {item.cantidad_total_alquilada}
                            </td>
                            <td className={`text-center fw-semibold ${nuncaAlquilado ? "text-muted" : "text-primary"}`}>
                              {item.cantidad_veces_alquilada}
                            </td>
                          </tr>
                        );
                      ...
                      <ul className="pagination pagination-sm mb-0">
                        ...
                        {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((pagina) => {
                          ...
                          if (
                            ...
                            return (
                              <li key={pagina} className="page-item disabled">
          ...
          <div className="card-body">
            ...
            <div className="row mb-4">
              ...
              <div className="col-md-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTipoContrato(
                      e.target.value as "todos" | "presupuestos" | "ordenes_trabajo"
                    )
                  }
                >
                  <option value="todos">Todos</option>
            ...
            {isLoadingContratos ? (
                    ...
                    <div className="col-md-6">
                      <div className="card border-success">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-success fw-bold">
                            ${contratos.reduce((sum, c) => sum + c.total, 0).toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                    ...
                    <tbody>
                      {contratos.map((contrato) => (
                        <tr key={`${contrato.tipo}-${contrato.id}`}>
                          <td>
                            <span className={`badge ${contrato.tipo === "orden_trabajo" ? "bg-success" : "bg-info"}`}>
                              {contrato.tipo === "orden_trabajo" ? "Orden" : "Presupuesto"}
                            </span>
                          </td>
                          <td className="fw-medium">{contrato.numero}</td>
                          <td>{contrato.cliente_nombre}</td>
                          <td className="small text-muted">{contrato.cliente_dni}</td>
                          <td className="small">
                            {format(new Date(contrato.fecha_creacion + "T00:00:00"), "dd/MM/yyyy", { locale: es })}
                          </td>
                          <td className="small">
                            {contrato.fecha_evento
                              ? format(new Date(contrato.fecha_evento + "T00:00:00"), "dd/MM/yyyy", { locale: es })
                              : "N/A"}
                          </td>
                          <td className="small text-muted">{contrato.categoria_evento}</td>
                          ...
                          <td>
                            <span
                              className={`badge ${
                                contrato.estado === "convertido_orden" || contrato.estado === "completada"
                                  ? "bg-success"
                                  : contrato.estado === "aprobado" || contrato.estado === "lista"
                                  ? "bg-primary"
                                  : contrato.estado === "rechazado" || contrato.estado === "cancelada"
                                  ? "bg-danger"
                          ...
                          <td className="text-center">{contrato.cantidad_items}</td>
                          <td className="text-center">
                            {contrato.seña_pagada !== null && contrato.seña_pagada !== undefined ? (
                          ...
                          <td className="text-center">
                            {contrato.saldo_pendiente !== null && contrato.saldo_pendiente !== undefined ? (
                              <span className={`fw-semibold ${contrato.saldo_pendiente > 0 ? "text-danger" : "text-success"}`}>
              ...
              <div className="text-center text-muted py-5">
                ...
                <h5 className="text-muted">No hay datos</h5>
                <p className="text-muted">
                  No se encontraron contratos para el rango de fechas seleccionado.
                  <br />
          ...
          <div className="card-body">
            ...
            {isLoadingRecibos ? (
                    ...
                    <div className="col-md-6">
                      <div className="card border-success">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-success fw-bold">
                            ${recibos.reduce((sum, r) => sum + r.monto, 0).toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                    ...
                    <tbody>
                      {recibos.map((recibo) => (
                        <tr key={recibo.movimiento_id}>
                          <td className="small">
                            {(() => {
                              try {
                                ...
                                console.error("Error al formatear fecha:", recibo.fecha_hora, e);
                                return recibo.fecha_hora;
                              }
                            })()}
                          </td>
                          <td className="fw-medium">{recibo.presupuesto_numero}</td>
                          <td>{recibo.cliente_nombre}</td>
                          <td className="small text-muted">{recibo.cliente_dni}</td>
                          ...
                          <td className="small text-muted">{recibo.metodo_pago}</td>
                          <td className="small text-muted">{recibo.usuario_nombre}</td>
                          <td className="text-center">
                            <button
              ...
              <div className="text-center text-muted py-5">
                ...
                <h5 className="text-muted">No hay datos</h5>
                <p className="text-muted">
                  No se encontraron recibos para el rango de fechas seleccionado.
                  <br />
          ...
          <div className="card-body">
            ...
            <div className="row mb-4">
              ...
              <div className="col-md-4 d-flex align-items-end gap-2">
                ...
                {ingresosPorTipo && ingresosPorTipo.ingresos_por_tipo && ingresosPorTipo.ingresos_por_tipo.length > 0 && (
                  <button
              ...
              <div>
                    ...
                    <div className="col-md-4">
                      <div className="card border-primary">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-primary fw-bold">
                            ${ingresosPorTipo.total_general.toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                ...
                <div className="table-responsive">
                  <table className="table table-hover table-striped">
                    ...
                    <tbody>
                      {ingresosPorTipo.ingresos_por_tipo.map((item: any, index: number) => {
                        const porcentaje = ingresosPorTipo.total_general > 0
                          ? ((item.total / ingresosPorTipo.total_general) * 100).toFixed(2)
                          : "0.00";
                        return (
                          <tr key={index}>
                            <td className="fw-medium">
                              ...
                              {item.metodo === "EFECTIVO" ? "Efectivo" :
                               item.metodo === "DEBITO" ? "Débito" :
                               item.metodo === "CREDITO" ? "Crédito" :
                               item.metodo === "BILLETERA_VIRTUAL" ? "Billetera Virtual" :
                               item.metodo === "TRANSFERENCIA" ? "Transferencia" :
                               item.metodo === "SIN_METODO" ? "Sin Método" :
                               item.metodo}
                            </td>
                            <td className="text-center">
      ...
      {selectedReporte === "stock_estado" && (
        <div className="card">
          <div className="card-header">
            ...
            <p className="text-muted small mb-0">
              Inventario agrupado por estado: Salón, Clientes, Modista, Lavandería, Vendido
            </p>
          </div>
          <div className="card-body">
            {/* Botones de acción */}
            <div className="mb-4 d-flex justify-content-between align-items-center">
              ...
              {stockPorEstado && stockPorEstado.stock_por_estado && stockPorEstado.stock_por_estado.length > 0 && (
                <button
            ...
            {isLoadingStock ? (
              ...
              <div>
                ...
                {stockPorEstado.stock_por_estado.map((item: any, index: number) => {
                  const porcentajeProductos = stockPorEstado.total_productos > 0
                    ? ((item.cantidad_productos / stockPorEstado.total_productos) * 100).toFixed(2)
                    : "0.00";
                  ...
                  const totalPaginas = Math.ceil(productos.length / productosPorPaginaStock);
                  const indiceInicio = (paginaActual - 1) * productosPorPaginaStock;
                  ...
                  return (
                    <div key={index} className="card mb-3">
                      <div className="card-header d-flex justify-content-between align-items-center">
                        <div>
                          <span className={`badge ${getEstadoBadge(item.estado)} me-2`}>
                      ...
                      <div className="card-body">
                              ...
                              {productosPaginados.length > 0 ? (
                                productosPaginados.map((producto: any) => (
                                  <tr key={producto.id}>
                                    ...
                                    <td className="small text-muted">{producto.tela}</td>
                                    <td>
                                    ...
                                    <td className="text-center">
                                      <select
                                        ...
                                        onChange={(e) => actualizarEstadoProducto(producto.id, e.target.value)}
                        ...
                        {totalPaginas > 1 && (
                          <div className="d-flex justify-content-between align-items-center mt-3">
                            <div className="text-muted small">
                              Mostrando {indiceInicio + 1} - {Math.min(indiceFin, productos.length)} de {productos.length} productos
                            </div>
                            <nav>
                              <ul className="pagination pagination-sm mb-0">
                                <li className={`page-item ${paginaActual === 1 ? "disabled" : ""}`}>
      ...
      {selectedReporte === "stock_por_linea" && (
        <div className="card">
          <div className="card-header">
            ...
            <p className="text-muted small mb-0">
              Inventario agrupado por línea, valorizado a costo y precio de venta
            </p>
          </div>
          <div className="card-body">
            {/* Botones de acción */}
            <div className="mb-4 d-flex justify-content-between align-items-center">
              ...
              {stockPorLinea && stockPorLinea.stock_por_linea && stockPorLinea.stock_por_linea.length > 0 && (
                <button
              ...
              <div>
                      ...
                      <div className="card border-warning">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-warning fw-bold">
                            ${stockPorLinea.total_valor_costo.toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                    ...
                    <div className="col-md-3">
                      <div className="card border-info">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-info fw-bold">
                            ${stockPorLinea.total_valor_venta.toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                    ...
                    <div className="col-md-3">
                      <div className="card border-success">
                        <div className="card-body text-center">
                          ...
                          <div className="h3 text-success fw-bold">
                            ${(stockPorLinea.total_valor_venta - stockPorLinea.total_valor_costo).toLocaleString("es-AR", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                ...
                <div className="table-responsive">
                  <table className="table table-hover table-striped">
                    ...
                    <tbody>
                      {stockPorLinea.stock_por_linea.map((item: any, index: number) => {
                        const porcentajeProductos = stockPorLinea.total_productos > 0
                          ? ((item.cantidad_productos / stockPorLinea.total_productos) * 100).toFixed(2)
                          : "0.00";
                        const porcentajeValor = stockPorLinea.total_valor_venta > 0
                          ? ((item.valor_venta / stockPorLinea.total_valor_venta) * 100).toFixed(2)
                          : "0.00";
                        const diferencia = item.valor_venta - item.valor_costo;
                        ...
                        return (
                          <tr key={index}>
                            ...
                            <td className={`text-end fw-semibold ${diferencia >= 0 ? "text-success" : "text-danger"}`}>
                    ...
                    <tfoot className="table-secondary">
                      <tr>
                        ...
                        <td className={`text-end fw-bold ${(stockPorLinea.total_valor_venta - stockPorLinea.total_valor_costo) >= 0 ? "text-success" : "text-danger"}`}>
                          ${(stockPorLinea.total_valor_venta - stockPorLinea.total_valor_costo).toLocaleString("es-AR", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
