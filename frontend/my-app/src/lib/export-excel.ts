...
/**
 * Genera un .xlsx con varias hojas y dispara la descarga en el navegador.
 */
export async function downloadExcelFromSheets(
  fileNameBase: string,
  sheets: Array<{
    name: string;
    rows: ExcelCell[][];
  ...
  sheets.forEach(({ name, rows, columnWidth = 20 }) => {
    const ws = wb.addWorksheet(name.slice(0, 31) || "Datos");
    rows.forEach((row) => {
      ws.addRow(row.map((c) => (c === null || c === undefined ? "" : c)));
    });
    const maxCols = rows.reduce((m, r) => Math.max(m, r.length), 0);
