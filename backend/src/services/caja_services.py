...
class CajaServices:
    ...
    def _determinar_categoria_automatica(self, tipo: str, origen: str) -> str:
        ...
        if tipo == "INGRESO":
            ...
            elif origen.startswith("PAGO_CUENTA_CORRIENTE:"):
    ...
    def get_movimientos_rango(
        ...
        """Movimientos de caja diaria en un rango (ingresos y egresos) para exportación."""
        try:
            sucursal = Sucursal.get(id=sucursal_id)
            if not sucursal:
                raise HTTPException(status_code=404, detail="Sucursal no encontrada")
            ...
            for cm in CajaMovimiento.select():
                ...
                if cm.metodo_pago_configurable:
                    metodo_movimiento_display = cm.metodo_pago_configurable.nombre
                    if cm.submetodo_pago:
                        metodo_movimiento_display = (
                            f"{cm.metodo_pago_configurable.nombre} - {cm.submetodo_pago.nombre}"
                        )
                elif cm.payment_method:
                    if hasattr(cm.payment_method, "value"):
                        metodo_movimiento_display = cm.payment_method.value
            ...
            for cm in movimientos_filtrados:
                ...
                categoria = cm.categoria if cm.categoria else self._determinar_categoria_automatica(
                    cm.tipo, cm.origen
                )
                cuenta_destino_nombre = cm.cuenta_destino.nombre_titular if cm.cuenta_destino else None
                origen_display = self._origen_con_cliente(cm.origen) if cm.origen else (cm.origen or "")
                metodo_key = metodo_pago_display or "SIN_METODO"
                if metodo_key not in resumen_por_metodo:
                    resumen_por_metodo[metodo_key] = 0.0
                if cm.tipo == "INGRESO":
                    resumen_por_metodo[metodo_key] += cm.monto
                    total_general += cm.monto
                    importe_neto = cm.monto
                else:
                    resumen_por_metodo[metodo_key] -= cm.monto
                    total_general -= cm.monto
                    importe_neto = -cm.monto
            ...
            print(f"❌ Error al obtener movimientos de caja por rango: {e}")
            raise HTTPException(
                status_code=500, detail="Error al obtener movimientos de caja por rango"
            )
    ...
    def _get_descripcion_movimiento_presupuesto(self, origen: str) -> str:
        ...
        if origen.startswith("SEÑA_PRESUPUESTO:"):
            ...
            cliente_id_str = origen.replace("PAGO_CUENTA_CORRIENTE:", "").strip()
            try:
                cliente = Cliente.get(id=int(cliente_id_str))
                if cliente:
                    nombre = f"{cliente.apellido} {cliente.nombre}".strip()
                    return f"Pago a cuenta corriente - {nombre}"
            except (ValueError, TypeError):
                pass
            return "Pago a cuenta corriente"
        else:
            return origen
