...
class CajaChicaService:
    ...
    def obtener_movimientos_rango(
        ...
        """Movimientos de caja chica en un rango de fechas (para exportación)."""
        ...
        movimientos = [
            cc
            for cc in movimientos
            if cc.sucursal.id == sucursal_id_val
            and fecha_desde <= cc.fecha.date() <= fecha_hasta
        ]
        movimientos.sort(key=lambda cc: cc.fecha, reverse=True)
        ...
        filas = []
        for mov in movimientos:
            mapeado = self._mapear_movimiento(mov)
            if mov.estado == EstadoMovimientoCajaChica.RECHAZADO:
                mapeado["impacta_total"] = False
            else:
                mapeado["impacta_total"] = True
                if mov.tipo == TipoMovimientoCajaChica.INGRESO:
                    total_ingresos += mov.monto
                elif mov.tipo == TipoMovimientoCajaChica.EGRESO:
                    total_egresos += mov.monto
            filas.append(mapeado)

        return {
            "movimientos": filas,
            "total_ingresos": float(total_ingresos),
            "total_egresos": float(total_egresos),
            "total_neto": float(total_ingresos - total_egresos),
