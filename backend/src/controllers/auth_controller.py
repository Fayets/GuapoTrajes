...
async def verify_token(token: str):
    try:
        ...
        user = servicio.buscar_usuario_por_id(int(user_id))
...
async def register(user: schemas.UserCreate, current_user=Depends(get_current_user)):
    try:
...
async def login(request: schemas.LoginRequest):
    ...
    if not username and not email:
        raise HTTPException(
            status_code=400,
            detail="Se requiere un nombre de usuario o un email",
        )
