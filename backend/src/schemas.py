...
class EtiquetasInventarioRegistrarRequest(BaseModel):
    producto_ids: List[int]


class EtiquetasInventarioResetRequest(BaseModel):
    ...
    tela_id: Optional[int] = None
    color_id: Optional[int] = None
    # Obligatorio cuando todos=true: frase exacta RESETEAR_TODO_INVENTARIO
    confirmacion_global: Optional[str] = None


class EtiquetasInventarioStatsResponse(BaseModel):
    total: int
    impresos: int
    pendientes: int
...
class ProductPrecioCampo(str):
    PRECIO_ALQUILER_LISTA = "precio_alquiler_lista"
    PRECIO_ALQUILER_EFECTIVO = "precio_alquiler_efectivo"
    PRECIO_VENTA_NUEVO_LISTA = "precio_venta_nuevo_lista"
    PRECIO_VENTA_NUEVO_EFECTIVO = "precio_venta_nuevo_efectivo"
    PRECIO_VENTA_MEDIO_USO = "precio_de_venta_medio_uso"
    PRECIO_VENTA = "precio_venta"
    PRECIO_LIQUIDACION = "precio_liquidacion"


class ProductBulkPriceUpdateRequest(BaseModel):
    modo: Literal["porcentaje", "monto_fijo"]
    valor: float
    campos: List[str] = Field(..., min_length=1)
    linea_ids: Optional[List[int]] = None
    talle_ids: Optional[List[int]] = None
    tela_ids: Optional[List[int]] = None
    color_ids: Optional[List[int]] = None
    estado: Optional[str] = None
    solo_ids: Optional[List[int]] = None


class ProductBulkPriceUpdateResponse(BaseModel):
...
class ProductoReservadoSchema(BaseModel):
    ...
    requiere_modista: bool = False
    notas_modista: Optional[str] = None


class ProductoReservadoModistaUpdateSchema(BaseModel):
    requiere_modista: bool = Field(
        ..., description="Indica si la prenda debe ir a modista para esta orden"
    )
    notas_modista: Optional[str] = Field(
        None,
        max_length=2000,
        description="Detalle del trabajo a realizar en modista",
    )

    @field_validator("notas_modista", mode="before")
    @classmethod
    def normalizar_notas_modista(cls, v):
        if v is None:
            return None
        s = str(v).strip()
        return s or None
...
class OrdenTrabajoCreateSchema(BaseModel):
    presupuesto_id: int
    seña_pagada: float = Field(..., gt=0, description="Monto total de la seña (incluye lo tomado de cuenta corriente si aplica)")
    credito_aplicado: float = Field(
        default=0,
        ge=0,
        description="Parte de la seña tomada del saldo a favor del cliente",
    )
    ...
    cuenta_destino_id: Optional[int] = Field(None, description="Obligatoria si hay importe que ingresa en caja")
    conjunto_separado: bool = Field(
        default=False,
        description="Indica que el conjunto ya quedó separado en perchero al confirmar la seña",
    )
    ...
    def validate_payment_method_present(self):
        credito = float(self.credito_aplicado or 0)
        if credito > float(self.seña_pagada) + 1e-9:
            raise ValueError("credito_aplicado no puede superar seña_pagada")
        monto_efectivo = float(self.seña_pagada) - credito
        if monto_efectivo > 1e-9:
            ...
            if not has_old_method and not has_new_method:
                raise ValueError(
                    "Debe proporcionar payment_method/metodo_pago (compatibilidad) o metodo_pago_id (nuevo sistema) cuando hay importe en caja"
                )
            if not self.cuenta_destino_id:
                raise ValueError("La cuenta destino es obligatoria cuando hay importe registrado en caja")
        return self
...
class PagoSaldoPendienteSchema(BaseModel):
    monto_pagado: float = Field(..., gt=0, description="Monto total aplicado al saldo pendiente de la orden")
    credito_aplicado: float = Field(
        default=0,
        ge=0,
        description="Parte del pago tomada del saldo a favor del cliente (cuenta corriente)",
    )
    ...
    cuenta_destino_id: Optional[int] = Field(None, description="ID de la cuenta destino (obligatoria si hay importe en caja)")
    motivo_recibo: Optional[str] = Field(None, description="Motivo del recibo: Seña, Alquilado, Cancelación, etc.")

    @model_validator(mode='after')
    def validate_payment_method_present(self):
        credito = float(self.credito_aplicado or 0)
        if credito > float(self.monto_pagado) + 1e-9:
            raise ValueError("credito_aplicado no puede superar monto_pagado")
        monto_efectivo = float(self.monto_pagado) - credito
        if monto_efectivo > 1e-9:
            has_old_method = self.payment_method is not None
            has_new_method = self.metodo_pago_id is not None
            if not has_old_method and not has_new_method:
                raise ValueError(
                    "Debe proporcionar payment_method (compatibilidad) o metodo_pago_id (nuevo sistema) cuando hay importe en caja"
                )
            if not self.cuenta_destino_id:
                raise ValueError("La cuenta destino es obligatoria cuando hay importe registrado en caja")
        return self
...
class VentaCreate(BaseModel):
    ...
    descuento_porcentaje: Optional[float] = None  # Descuento sobre el total (como en presupuestos)
    # Campos de descuento extra (obligatorio motivo si supera el máximo estándar)
    extra_discount_percentage: Optional[float] = None
    extra_discount_amount: Optional[float] = None
    extra_discount_reason: Optional[str] = None
...
class PagoAdicionalRequest(BaseModel):
    ...
    credito_aplicado: float = Field(
        default=0,
        ge=0,
        description="Parte del monto tomada del saldo a favor del cliente",
    )
    ...
    cuenta_destino_id: Optional[int] = None  # Obligatoria si hay importe en caja

    @model_validator(mode='after')
    def validate_payment_method_present(self):
        credito = float(self.credito_aplicado or 0)
        if credito > float(self.monto) + 1e-9:
            raise ValueError("credito_aplicado no puede superar el monto total")
        monto_efectivo = float(self.monto) - credito
        if monto_efectivo > 1e-9:
            has_old_method = self.metodo_pago is not None
            has_new_method = self.metodo_pago_id is not None
            if not has_old_method and not has_new_method:
                raise ValueError(
                    "Debe proporcionar metodo_pago (compatibilidad) o metodo_pago_id (nuevo sistema) cuando hay importe en caja"
                )
            if not self.cuenta_destino_id:
                raise ValueError("La cuenta destino es obligatoria cuando hay importe registrado en caja")
        return self
...
class CreditoManualRequest(BaseModel):
    cliente_id: int
    monto: float = Field(..., gt=0)
    concepto: str = Field(..., min_length=1, max_length=500)
    registrar_en_caja: bool = Field(
        True,
        description="Si es un pago real del cliente, registra ingreso en caja diaria además del crédito en cuenta corriente",
    )
    ...
    @model_validator(mode="after")
    def validar_campos_caja(self):
        if self.registrar_en_caja:
            if not self.metodo_pago_id:
                raise ValueError(
                    "Debe indicar metodo_pago_id cuando el abono registra ingreso en caja"
                )
            if not self.cuenta_destino_id:
                raise ValueError(
                    "Debe indicar cuenta_destino_id cuando el abono registra ingreso en caja"
                )
        return self
