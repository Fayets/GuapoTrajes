...
- `ENV`: `development` habilita bootstrap automático de SUPER_ADMIN; en producción usar `production`
- `BOOTSTRAP_ADMIN`: `true` crea usuario SUPER_ADMIN inicial aunque `ENV` no sea development
- `CORS_ORIGINS`: orígenes permitidos separados por coma (ej. `http://localhost:3000,https://tu-app.netlify.app`)

El bootstrap de SUPER_ADMIN (`DesarrolloGuapo` / `Roma123!`) **solo corre** con `ENV=development` o `BOOTSTRAP_ADMIN=true`. No se crea automáticamente en producción.

### Impresión de etiquetas (dos subsistemas)

- **Operativo (producción)**: QZ Tray + fallback navegador en órdenes, presupuestos, reportes, productos, lavandería y modista. Librerías en `frontend/my-app/src/lib/imprimir-etiqueta-*.ts`.
- **Provisorio (migración inventario)**: página `/etiquetas-inventario`, controlada por `NEXT_PUBLIC_HABILITAR_ETIQUETAS_MIGRACION` (activa salvo `false`). Reset global requiere SUPER_ADMIN y confirmación `RESETEAR_TODO_INVENTARIO`.
